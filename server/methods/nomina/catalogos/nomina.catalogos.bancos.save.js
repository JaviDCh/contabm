...
Meteor.methods(
{
    'nomina.catalogos.bancos.save': function (items) {
        ...
        inserts.forEach(function (item) {
            ...
            item.banco = savedItem.banco     // recuperamos el id (pk) de la cuenta; pues se le asignó un valor en sql ...
        ...
        updates.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                Bancos_sql.update(item_sql, { where: { banco: item_sql.banco }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
