...
angular.module("contabm").controller("Catalogos_Nomina_bancos_Controller", ['$scope', '$meteor', function ($scope, $meteor) {
    ...
    $scope.deleteItem = function (item) {
        if (item.docState && item.docState === 1) {
            ...
            lodash.remove($scope.bancos, (x) => { return x.banco === item.banco; });
        }
    ...
    $scope.nuevo = function () {
        ...
        let ultimo = lodash.maxBy($scope.bancos, 'banco'); 
        let proximoID = 0; 

        if (!ultimo) { 
            proximoID = 1; 
        } else { 
            proximoID = ultimo.banco + 1; 
        }
    ...
    $scope.save = function () {
        ...
        let editedItems = lodash.filter($scope.bancos, function (item) { return item.docState; });
        ...
        Meteor.call('nomina.catalogos.bancos.save', editedItems, (err, saveMethodResult) => {
            ...
            if (err) {
                ...
                $scope.items_ui_grid.data = $scope.bancos;
            ...
            $scope.helpers({
                bancos: () => {
                    return Bancos.find({}, { sort: { banco: 1 } });
                }
            })
